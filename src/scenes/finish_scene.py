...
class FinishScene(Scene):
    ...
    def __init__(self, title_font: pygame.Font, text_font: pygame.Font, final_time: tuple[int, int], number_people: tuple[int, int], has_win: str):
        ...
        #init win render
        if has_win == "player":
            self.has_win_render = text_font.render("Vous avez gagné", True, BLACK)
        elif has_win == "ia":
            self.has_win_render = text_font.render("Vous avez perdu", True, BLACK)
        else:
            self.has_win_render = text_font.render("Égalité", True, BLACK)
    ...
    def draw(self, draw_surface: pygame.Surface, *args: list, **kwargs: dict):
        ...
        #draw button 
        pygame.draw.rect(draw_surface, BLUE, self.finish_button, border_radius=5)
